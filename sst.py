# From https://github.com/cgpotts/cs224u
...
def build_dataset(dataframes, phi, vectorizer=None, vectorize=True, text_field="sentence", label_field="label"):
    """
    ...
    raw_examples = getattr(df, text_field).to_numpy().tolist()

    feat_dicts = getattr(df, text_field).apply(phi).to_numpy().tolist()
...
def build_rnn_dataset(dataframes, tokenizer=lambda s: s.split(), text_field="sentence", label_field="label"):
    """
    ...
    if isinstance(dataframes, (list, tuple)):
        ...
        df = dataframes
    X = list(getattr(df, text_field).apply(tokenizer))
    y = getattr(df, label_field).to_numpy().tolist()
    return X, y
